...
with tab1:
    ...
    # ══════════════════════════════════════════════════════════
        ...
        if not usable_batch:
            ...
            if run_batch_btn and _ready_sentences and batch_model:
                ...
                for i, sentence in enumerate(batch_sentences, start=1):
                    ...
                    out_filename = generate_batch_filename(i, batch_model, det_lang, sentence)
                ...
                if save_batch_json:
                    batch_json_path = (
                        BATCH_OUTPUT_DIR
                        / f"batch_{batch_model[:20].replace(' ','_')}_{int(time.time())}.json"
                    )
            ...
            if "batch_results_df" in st.session_state:
                ...
                if ok_wavs:
                    ...
                    st.download_button(
                        ...
                        file_name=f"batch_audio_{batch_model[:20].replace(' ','_')}_{int(time.time())}.zip",
